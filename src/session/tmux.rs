...
impl RuntimeProvider for TmuxProvider {
    ...
    fn create(
        ...
        if let Err(err) = self.set_label(&tmux_session_name, project_name, session_name) {
    ...
    fn attach(&self, runtime_id: &str) -> Result<()> {
        let status = Command::new("tmux")
            .args(["attach-session", "-t", runtime_id])
            .status()
    ...
    fn kill(&self, runtime_id: &str) -> Result<()> {
        let status = Command::new("tmux")
            .args(["kill-session", "-t", runtime_id])
            .status()
    ...
    fn send_keys(&self, runtime_id: &str, text: &str) -> Result<()> {
        for line in text.lines() {
            ...
            let mut args = vec!["send-keys", "-t", runtime_id];
            if !line.is_empty() {
                args.push(line);
            }
            args.push("Enter");
    ...
    fn set_label(
        ...
        tmux_set_session_option(runtime_id, "status-left", &status_left)?;
        tmux_set_session_option(runtime_id, "status-left-length", &status_left_len)?;
        tmux_set_session_option(runtime_id, "set-titles", "on")?;
        tmux_set_session_option(runtime_id, "set-titles-string", &label)?;
